...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
	...
	pthread_mutex_init(&self->krisimi,NULL);
	self->diabastike=0;
	pthread_cond_init(&(self->readOper),NULL);
	pthread_cond_init(&(self->writeOper),NULL);
	self->write = false;
...
void blockRead(DB* self){

	pthread_mutex_lock(&self->krisimi);//Lock: 1h krisimi perioxi
	while(self->diabastike > 0)
		pthread_cond_wait(&self->readOper,&self->krisimi);
	self->write = true;
}	
...
void giveSignal2(DB* self){
	pthread_mutex_lock(&self->krisimi);
	self->diabastike=self->diabastike-1;
	if(self->diabastike == 0){//teleiwse to diabasma
		pthread_cond_signal(&self->readOper);//signal to write gia na synexisei
	}
	pthread_mutex_unlock(&self->krisimi);
}
...
int db_get(DB* self, Variant* key, Variant* value)
{
	...
	//Lock : 2h krisimi perioxi
	blockWrite(self);
	
	if (memtable_get(self->memtable->list, key, value) == 1){
		met = 1;
	}else{
		met = sst_get(self->sst, key, value);
	}
	
	//Unlock : 2h krisimi perioxi
	giveSignal2(self);
	return met;
